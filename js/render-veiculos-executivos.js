...
async function renderExecutivos() {
  const container = document.getElementById('admin-executivos')
  if (!container) return
  ...
  try {
    ...
    if (!data || data.length === 0) {
      container.innerHTML = '<p class="text-gray-500 text-center">Nenhum veículo executivo cadastrado no painel admin.</p>'
      return
    }
      ...
      return `
        <div class="premium-card animate-on-scroll">
          ...
          <div class="p-6">
            ...
            <button data-whatsapp="fleet-detail" data-vehicle="${v.modelo}" data-service="Orçamento Executivo" class="premium-button w-full justify-center">
    ...
    console.error('Erro ao renderizar executivos:', e)
  }
}

document.addEventListener('DOMContentLoaded', renderExecutivos)
window.addEventListener('i18n:change', renderExecutivos)
